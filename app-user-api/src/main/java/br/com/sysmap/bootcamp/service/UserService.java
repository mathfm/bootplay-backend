...
public class UserService implements UserDetailsService {
  ...
  private UserEntity createUser(UserDto userCreatedDto) {
    UserEntity user = UserEntity.builder()
        ...
        .name(userCreatedDto.getName()).email(userCreatedDto.getEmail()).build();
    return userRepository.save(user);
  }

  private void createWalletUser(UserEntity user) {
    WalletEntity wallerUser = WalletEntity.builder().balance(new BigDecimal("99.99")).points(0L)
        .lastUpdated(LocalDateTime.now()).user(user).build();
  ...
  public UserEntity updateUser(UserDto userDto) {
    ...
    userUpdateEntity = userUpdateEntity.toBuilder().name(userDto.getName())
        .email(userDto.getEmail()).password(this.passwordEncoder.encode(userDto.getPassword()))
        .build();
  ...
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    ...
    return optionalUser.map(
            user -> new User(user.getEmail(), user.getPassword(), new ArrayList<GrantedAuthority>()))
  ...
  public AuthDto auth(AuthDto authDto) {
    ...
    return AuthDto.builder().email(user.getEmail())
        .token(Base64.getEncoder().withoutPadding().encodeToString(password.getBytes()))
        .id(user.getId()).build();
  }
